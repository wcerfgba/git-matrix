// @flow
var CELL_HEIGHT_REMS, bodyMaxWidthRems, columnWidthPercentage, elMap, fileRows, floor, header, layout, matrixToHtml, numCols, range, scoreCellStyle, stylesheet, table, tableHeading, textColorHsl;
...
tableHeading = ({scope}) => {
  return (...headings) => {
    return headings.map((heading) => {
      return `<th scope='${scope}' data-tooltip='${heading}'>${heading}</th>`;
    }).join('\n');
  };
};
