...
tableHeading = ({scope}) => {
  return (...headings) => {
    return headings.map((heading) => {
      return `<th scope='${scope}' title='${heading}'>${heading}</th>`;
    }).join('\n');
  };
};
